...
- [ ] If you do not already have them, you need to [install 'git'](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git),
create a [GitHub account](https://github.com), and set up [SSH keys](https://docs.github.com/en/authentication/connecting-to-github-with-ssh) to allow you to clone repositories using SSH.
...
- [ ] (recommended) start a new branch; if you will do several projects, it is recommended to put each branch in a separate directory using `git worktree`.  This allows them to share one copy of the `.git` directory, as well as `protaxFungi`.
  ...
  **Or**, if you only plan on doing one project, you can just create the branch in the main directory:
  ...
  ```
- [ ] (Optional: for ITS fungal metabarcoding) download [protaxFungi](https://github.com/psomervuo/protaxfungi) and unzip into a sister directory (i.e., `../protaxFungi`).
  ...
  ```
- [ ] If you are **not** using the OptimOTU v7 Apptainer image (which already
  includes USEARCH 11 on `PATH`), download [USEARCH 11](https://github.com/rcedgar/usearch_old_binaries)
  into `bin/` and rename or link the executable to `bin/usearch`. Make sure it
  is executable. USEARCH 12 is not supported.
  ```sh
...
- [ ] Install R dependencies.  This can be done in several ways:  
  ...
  ```
  The conda environment file listed above includes only direct dependencies of OptimOTU.
  Occasionally a new version of an indirect dependency may cause something to break.
  In that case, you can instead use `conda env create -f conda/OptimOTU_v6_full.yaml`.
...
fastq files relative to the directory `sequences/01_raw`.
...
Before the first time you run, modify `run_crew.sh` and `slurm/puhti_crew.tmpl` to change the lines
...
It should be possible to run on other Slurm-based HPC systems by additional modification of `run_node.sh`, `run_crew.sh`, `run_crew.R`, and `slurm/puhti_crew.tmpl`.  The least portable element is the tykky containerization. In the future a Singularity container will be provided to make installation on other systems easier.
