...
title: ITS2 taxonomy-first metabarcoding pipeline
---
...
- [ ] clone the repository from github; ```git clone git@github.com:brendanf/sonja_spruce_logs.git```  
This will by default create a directory called `sonja_spruce_logs` for your project, but you can put it in a different directory using ```git clone git@github.com:brendanf/sonja_spruce_logs.git {name_of_directory}```
...
- [ ] download [usearch](https://drive5.com/usearch/) into `bin/`, and rename or link the executable to be called just `bin/usearch`. Make sure it is executable.
  ...
  cd ..
  ```
- [ ] download [reference data for the Unite sh_matching pipeline](https://files.plutof.ut.ee/public/orig/9C/FD/9CFD7C58956E5331F1497853359E874DEB639B17B04DB264C8828D04FA964A8F.zip) and unzip in `data/sh_matching_data`.  (Currently only `sanger_refs_sh.fasta` and `shs_out.txt` are used, so you can delete everything else.)
  ```sh
...
You may need to modify `run_clustermq.sh`, `run_clustermq.R` and `slurm/puhti_clustermq.tmpl`, for instance to change the project or tykky container (`.sh` and `.tmpl`) or the number of workers (`.R`).
