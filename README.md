...
- [ ] If you are **not** using the OptimOTU v7 Apptainer image (which
  already includes USEARCH 11 on `PATH`), download
  [USEARCH 11](https://github.com/rcedgar/usearch_old_binaries) into
  `bin/` and rename or link the executable to `bin/usearch`. Make sure
  it is executable. USEARCH 12 is not supported.

  ``` sh
  cd {name_of_project}/bin
  wget https://raw.githubusercontent.com/rcedgar/usearch_old_binaries/main/bin/usearch11.0.667_i86linux64 -O usearch
  chmod +x usearch
  cd ..
  ```
