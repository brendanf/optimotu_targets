...
dada_plan <- list(
  tar_map(
    ...
    tar_file_fast(
      ...
      purrr::pmap(
        ...
        unlist(),
      pattern = map(dada2_meta), # per seqrun and orientation
      iteration = "list"
    ),

    #### trim_read_counts_{.orient} ####
    # tibble:
    ...
    tar_map(
      ...
      tar_target(
        ...
        dada2::derepFastq(filtered, verbose = TRUE) %>%
          set_names(sub("(fwd|rev)_R[12]_filt\\.fastq\\.gz", "", filtered)),
  ...
  if (pipeline_options$orient == "mixed") {
    ...
    tar_target(
      seqtable_raw,
      dada2::makeSequenceTable(merged_fwd),
      pattern = map(merged_fwd), # per seqrun
      iteration = "list"
    )
  },
